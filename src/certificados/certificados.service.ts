...
export class CertificadosService {
  ...
  private readonly encryptionKey =
    process.env.CERTIFICATE_ENCRYPTION_KEY || 'fenix-cert-key-2024';
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'Formato de arquivo inválido. Use apenas .pfx ou .p12',
        );
      }
      ...
      if (file.size < 1024) {
        // 1KB
        throw new BadRequestException(
          'Arquivo muito pequeno. Verifique se é um certificado válido',
        );
      }
  ...
  private async extractCertificadoData(file: any, senha: string): Promise<any> {
    try {
      this.logger.log(
        'Tentando extrair dados do certificado usando serviço Java...',
      );

      // Primeiro, tentar usar o serviço Java
      const isJavaServiceAvailable =
      ...
      if (isJavaServiceAvailable) {
        ...
        try {
          const javaResult =
            await this.javaCertificadoService.validarCertificado(
              file.buffer,
              file.originalname,
              senha,
            );
          ...
          this.logger.log(
            `Certificado validado com sucesso via Java: ${javaResult.nome}`,
          );
          ...
          this.logger.warn(
            `Erro no serviço Java, usando fallback: ${javaError.message}`,
          );
        }
      } else {
        this.logger.warn(
          'Serviço Java não disponível, usando validação local...',
        );
      }
      ...
      this.logger.error(
        `Erro ao extrair dados do certificado: ${error.message}`,
      );
  ...
  private async extractCertificadoDataLocal(
    file: any,
    senha: string,
  ): Promise<any> {
    try {
      ...
      const isA3 =
        file.originalname.toLowerCase().includes('a3') ||
  ...
  private simulateCertificadoDataFallback(fileName: string): any {
    ...
    const isA3 =
      fileName.toLowerCase().includes('a3') ||
  ...
  async getCertificado(
    ...
    try {
      console.log(
        '🔍 Debug Service - Buscando certificado para companyId:',
        companyId,
      );
      ...
      if (!certificado) {
        console.log(
          '🔍 Debug Service - Nenhum certificado encontrado para companyId:',
          companyId,
        );
        return null;
      }
