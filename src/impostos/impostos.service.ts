...
import { CalcularImpostosPedidoDto, CalcularImpostosResponseDto, ImpostosItemCalculado } from './dto/calcular-impostos.dto';
import { ICMS_STRATEGIES, IPI_STRATEGIES, PIS_STRATEGIES, COFINS_STRATEGIES, baseCalc } from './strategies';
...
export class CalculadoraImpostosService {
  ...
  async calcularPedido(dto: CalcularImpostosPedidoDto): Promise<CalcularImpostosResponseDto> {
    ...
    console.log('🔍 Buscando configuração para:', { naturezaOperacaoId: dto.naturezaOperacaoId, ufDestino, ufOrigem });
    ...
    console.log('🔍 Configuração encontrada para UF destino:', confEstado ? 'Sim' : 'Não');
    ...
    if (!confEstado) {
      ...
      console.log('🔍 Configuração encontrada para UF origem:', confEstado ? 'Sim' : 'Não');
    }
    ...
    console.log('🔍 Todas as configurações disponíveis:', todasConfiguracoes.map(c => ({ uf: c.uf, habilitado: c.habilitado })));

    // Se ainda não encontrar, criar configuração padrão
    if (!confEstado) {
      console.log('⚠️ Configuração de imposto por estado não encontrada. Criando padrão.');
      confEstado = {
        id: 'default',
        naturezaOperacaoId: dto.naturezaOperacaoId,
        uf: ufDestino,
    ...
    console.log('✅ Configuração utilizada:', { ufOrigem, ufDestino, confEstado });
    ...
    const itensCalc: ImpostosItemCalculado[] = dto.itens.map((it) => {
      ...
      const freteItem = incluirFrete ? (totalFrete * (it.quantidade / totalQtd)) : 0;
      const despItem = totalDespesas * (it.quantidade / totalQtd);

      // Usar CSTs do item ou da configuração
      const cstIcms = (it.icmsCST || confEstado?.icmsCST || '00') as string;
      const cstIpi = (it.ipiCST || confEstado?.ipiCST || '00') as string;
      const cstPis = (it.pisCST || confEstado?.pisCST || '01') as string;
      const cstCofins = (it.cofinsCST || confEstado?.cofinsCST || '01') as string;

      // Calcular impostos usando as estratégias CST
      const { icms, icmsSt } = (ICMS_STRATEGIES[cstIcms] || ICMS_STRATEGIES.DEFAULT)({
        subtotal, desconto, freteItem, despItem, conf: confEstado, cbenef: it.cbenef,
      });

      const { ipi } = (IPI_STRATEGIES[cstIpi] || IPI_STRATEGIES.DEFAULT)({
        subtotal, desconto, freteItem, despItem, conf: confEstado, cbenef: it.cbenef, qtd: it.quantidade,
      });
      ...
      const cofins = (COFINS_STRATEGIES[cstCofins] || COFINS_STRATEGIES.DEFAULT)({
        subtotal, desconto, freteItem, despItem, conf: confEstado, cbenef: it.cbenef, qtd: it.quantidade,
      });
      ...
      const issElegivel = !(confEstado as any)?.issAcimaDe || baseIss >= Number((confEstado as any)?.issAcimaDe || 0);
      const issAliq = Number(((confEstado as any)?.issAliquota ?? (confEstado as any)?.issPorcentagem) || 0);
      const iss = issElegivel && baseIss > 0 && issAliq > 0 ? { 
        base: baseIss, 
        aliquota: issAliq, 
    ...
    const totalPedido = totalProdutos - totalDescontos + totalImpostos + Number(dto.valorFrete || 0) + Number(dto.despesas || 0);

    return { itens: itensCalc, totais: { totalProdutos, totalDescontos, totalImpostos, totalPedido } };
  }
}