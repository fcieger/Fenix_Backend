...
export class AddTransportadoraFields1734021000002
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!table?.findColumnByName('transportadora_id')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "transportadora_id" UUID`,
      );
    }

    if (!table?.findColumnByName('placa_veiculo')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "placa_veiculo" VARCHAR(10)`,
      );
    }

    if (!table?.findColumnByName('uf_placa')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "uf_placa" VARCHAR(2)`,
      );
    }

    if (!table?.findColumnByName('rntc')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "rntc" VARCHAR(20)`,
      );
    }

    if (!table?.findColumnByName('peso_liquido')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "peso_liquido" DECIMAL(10,2)`,
      );
    }

    if (!table?.findColumnByName('peso_bruto')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "peso_bruto" DECIMAL(10,2)`,
      );
    }

    if (!table?.findColumnByName('volume')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "volume" DECIMAL(10,3)`,
      );
    }

    if (!table?.findColumnByName('quantidade_volumes')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "quantidade_volumes" INTEGER`,
      );
    }

    if (!table?.findColumnByName('especie')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "especie" VARCHAR(50)`,
      );
    }

    if (!table?.findColumnByName('marca')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "marca" VARCHAR(50)`,
      );
    }

    if (!table?.findColumnByName('numeracao')) {
      await queryRunner.query(
        `ALTER TABLE "pedidos_venda" ADD COLUMN "numeracao" VARCHAR(100)`,
      );
    }
