...
export class NFeIntegrationController {
  ...
  async emitirNFe(
    ...
    this.logger.log(`Solicitação de emissão da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
    
    try {
      const response = await this.nfeIntegrationService.emitirNFe(nfeId, req.user.activeCompanyId);
  ...
  async sincronizarNFe(
    ...
    this.logger.log(`Solicitação de sincronização da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
  ...
  @Post('webhook')
  @HttpCode(HttpStatus.OK)
  async receberWebhook(@Body() webhookData: WebhookNFeDto): Promise<{ message: string; success: boolean }> {
    ...
    try {
      ...
      this.logger.log(`Webhook da NFe ${webhookData.nfeId} processado com sucesso`);
      return { 
  ...
  async getStatusNFe(
    ...
    this.logger.log(`Consultando status da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
  ...
  async cancelarNFe(
    ...
    this.logger.log(`Cancelando NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
    return this.nfeIntegrationService.cancelarNFe(nfeId, cancelarDto.justificativa, req.user.activeCompanyId);
  }
  ...
  async downloadXML(
    ...
    this.logger.log(`Download XML da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
    return this.nfeIntegrationService.downloadXML(nfeId, req.user.activeCompanyId);
  }
  ...
  async downloadPDF(
    ...
    this.logger.log(`Download PDF da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
    return this.nfeIntegrationService.downloadPDF(nfeId, req.user.activeCompanyId);
  }
  ...
  async downloadDANFE(
    ...
    this.logger.log(`Download DANFE da NFe ${nfeId} pela empresa ${req.user.activeCompanyId}`);
    return this.nfeIntegrationService.downloadDANFE(nfeId, req.user.activeCompanyId);
  }
  ...
  async consultarNFe(
    ...
    this.logger.log(`Consultando NFe ${chaveAcesso} pela empresa ${req.user.activeCompanyId}`);
    return this.nfeIntegrationService.consultarNFe(chaveAcesso, req.user.activeCompanyId);
  }
