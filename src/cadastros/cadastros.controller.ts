...
export class CadastrosController {
  ...
  async create(@Body() createCadastroDto: CreateCadastroDto, @Request() req) {
    try {
      ...
      console.log(
        ...
        JSON.stringify(createCadastroDto, null, 2),
      );
      ...
      console.log(
        'Dados para salvar (sem IDs):',
        JSON.stringify(cadastroData, null, 2),
      );

      const result = await this.cadastrosService.create(
        cadastroData,
        companyId,
      );
  ...
  async update(
    @Param('id') id: string,
    @Body() updateCadastroDto: Partial<CreateCadastroDto>,
    @Request() req,
  ) {
    return await this.cadastrosService.update(
      id,
      updateCadastroDto,
