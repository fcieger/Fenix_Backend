...
export function arred2(n: number) { return Number(n.toFixed(2)); }

export function baseCalc({ subtotal, desconto, freteItem, despItem, incluiFrete, incluiDesp, reducao }:
  { subtotal:number; desconto:number; freteItem:number; despItem:number; incluiFrete?:boolean; incluiDesp?:boolean; reducao?:number; }) {
...
export const ICMS_STRATEGIES: Record<string, (ctx: CtxBase) => OutICMS> = {
  '00': ({ conf, subtotal, desconto, freteItem, despItem }) => {
    ...
    return { icms: { base, aliquota: aliq, valor: arred2(base*aliq/100), cst: conf.icmsCST } };
  },
  '10': (ctx) => {
    // Tributada e com cobrança do ICMS por substituição tributária
    const baseIcms = baseCalc({ 
      subtotal: ctx.subtotal, desconto: ctx.desconto, freteItem: ctx.freteItem, despItem: ctx.despItem, 
    ...
    const icmsValor = baseIcms>0 && aliqIcms>0 ? arred2(baseIcms*aliqIcms/100) : 0;

    // ST: Base com MVA, redução e alíquota ST
    const baseComMVA = arred2(baseIcms * (1 + Number(ctx.conf.icmsMVA||0)/100));
    const baseSt = arred2(baseComMVA * (1 - Number(ctx.conf.icmsStReducaoBase||0)/100));
    const aliqSt = Number(ctx.conf.icmsStAliquota || 0);
    let stValor = baseSt>0 && aliqSt>0 ? arred2(baseSt*aliqSt/100) : 0;
    stValor = Math.max(0, arred2(stValor - icmsValor));

    return {
      icms: baseIcms>0 && aliqIcms>0 ? { base: baseIcms, aliquota: aliqIcms, valor: icmsValor, cst: ctx.conf.icmsCST } : undefined,
      icmsSt: baseSt>0 && aliqSt>0 ? { base: baseSt, aliquota: aliqSt, valor: stValor, cst: ctx.conf.icmsCSTSt || ctx.conf.icmsCST } : undefined
    };
  },
  ...
  '30': (ctx) => {
    ...
    const baseSt = arred2(baseComMVA * (1 - Number(ctx.conf.icmsStReducaoBase||0)/100));
    const aliqSt = Number(ctx.conf.icmsStAliquota || 0);
    const stValor = baseSt>0 && aliqSt>0 ? arred2(baseSt*aliqSt/100) : 0;
  ...
  '51': (ctx) => {
    ...
    const percentualDiferimento = Number(ctx.conf.icmsPercentualDiferimento || 100);
    ...
    return { 
      icms: base && aliq > 0 ? { 
        ...
        valorDiferido: arred2(valorDiferido * percentualDiferimento / 100),
  ...
  '90': (ctx) => {
    ...
    // Pode ter ST também
    const baseComMVA = arred2((base || 0) * (1 + Number(ctx.conf.icmsMVA||0)/100));
    const baseSt = arred2(baseComMVA * (1 - Number(ctx.conf.icmsStReducaoBase||0)/100));
    const aliqSt = Number(ctx.conf.icmsStAliquota || 0);
    const stValor = baseSt>0 && aliqSt>0 ? arred2(baseSt*aliqSt/100) : 0;

    return {
      icms: base && aliq > 0 ? { base, aliquota: aliq, valor, cst: ctx.conf.icmsCST } : undefined,
      icmsSt: baseSt>0 && aliqSt>0 ? { base: baseSt, aliquota: aliqSt, valor: stValor, cst: ctx.conf.icmsCSTSt || ctx.conf.icmsCST } : undefined
    };
  },
...
// ===== IPI STRATEGIES =====
export const IPI_STRATEGIES: Record<string, (ctx: CtxBase & { qtd?: number }) => OutIPI> = {
  '00': ({ conf, subtotal, desconto, freteItem, despItem }) => {
  ...
  '01': ({ conf, subtotal, desconto, freteItem, despItem }) => {
    ...
    return IPI_STRATEGIES['00']({ conf, subtotal, desconto, freteItem, despItem });
  },
  ...
  '04': ({ conf, subtotal, desconto, freteItem, despItem }) => {
    // Entrada tributada com alíquota por unidade de produto
    return IPI_STRATEGIES['03']({ conf, subtotal, desconto, freteItem, despItem });
  },
  ...
  '99': ({ conf, subtotal, desconto, freteItem, despItem, qtd }) => {
    ...
    if (conf.ipiAliquotaUnidade && qtd) {
      ...
      return { ipi: { base: 0, aliquotaUnidade: Number(conf.ipiAliquotaUnidade), valor, cst: conf.ipiCST, qtdTributada: qtd } };
    }
    return IPI_STRATEGIES['00']({ conf, subtotal, desconto, freteItem, despItem, qtd });
  },
  DEFAULT: (ctx) => IPI_STRATEGIES['00'](ctx)
};

// ===== PIS STRATEGIES =====
export const PIS_STRATEGIES: Record<string, (ctx: CtxBase & { qtd?: number }) => ImpostoValor|undefined> = {
  '01': ({ conf, subtotal, desconto, freteItem, despItem, cbenef }) => {
    ...
    return { base, aliquota: aliq, valor: arred2(base*aliq/100), cst: conf.pisCST, cbenef };
  },
  '02': (ctx) => {
  ...
  '03': ({ conf, qtd, cbenef }) => {
    ...
    return { base: 0, aliquotaUnidade: Number(conf.pisAliquotaUnidade), valor, cst: conf.pisCST, cbenef, qtdTributada: qtd };
  },
  '04': () => {
...
// ===== COFINS STRATEGIES =====
export const COFINS_STRATEGIES: Record<string, (ctx: CtxBase & { qtd?: number }) => ImpostoValor|undefined> = {
  '01': ({ conf, subtotal, desconto, freteItem, despItem, cbenef }) => {
    // Operação tributável - Base de cálculo = valor da operação alíquota normal (cumulativo/não cumulativo)
    const base = baseCalc({ 
      subtotal, desconto, freteItem, despItem, 
    ...
    if (base<=0 || aliq<=0) return undefined;
    return { base, aliquota: aliq, valor: arred2(base*aliq/100), cst: conf.cofinsCST, cbenef };
  },
  '02': (ctx) => {
    // Operação tributável - Base de cálculo = valor da operação (alíquota diferenciada)
  ...
  '03': ({ conf, qtd, cbenef }) => {
    ...
    return { base: 0, aliquotaUnidade: Number(conf.cofinsAliquotaUnidade), valor, cst: conf.cofinsCST, cbenef, qtdTributada: qtd };
  },
  '04': () => {
    // Operação tributável - Tributação monofásica (alíquota zero)
    return undefined;
